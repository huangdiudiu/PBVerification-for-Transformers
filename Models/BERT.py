...
class BERT:
    ...
    def step(self, batch, is_train=False, infer_grad=False,embeddings=None,before_norm_embeddings=None):
        input_ids, input_mask, segment_ids, features = self.get_input(batch)
        label_ids = torch.tensor([f.label_id for f in features], dtype=torch.long)
        label_ids = label_ids.to(self.device)

        if before_norm_embeddings is not None:
            before_norm_embeddings,delta=before_norm_embeddings
            before_norm_embeddings=before_norm_embeddings+delta
        
        if is_train:
            self.model.train()
            logits, embedding_output, encoded_layers, attention_scores, attention_probs, \
                self_output, pooled_output,QK_output = self.model(
            ...
            with grad:
                logits, embedding_output, encoded_layers, attention_scores, attention_probs, \
                    self_output, pooled_output,QK_output = self.model(
                        input_ids, segment_ids, input_mask, labels=None,embeddings=embeddings,before_norm_embeddings=before_norm_embeddings)
        ...
        if infer_grad:
            if before_norm_embeddings is not None:
                output_logits = logits[np.arange(logits.shape[0]), label_ids]
                non_output_logits=logits[np.arange(logits.shape[0]), 1-label_ids]
                gradients = torch.autograd.grad(non_output_logits-output_logits, before_norm_embeddings)[0]
                #gradients=torch.autograd.grad(-loss, before_norm_embeddings)[0]
            else:
