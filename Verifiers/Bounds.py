...
class Bounds:
    ...
    # def get_bounds_xy_bi(self, l_x, u_x, l_y, u_y, debug=False):
    #     self_lenth=l_x.shape[0]
    #     other_lenth=l_y.shape[0]
    #     d_model=l_x.shape[1]
    #     if self.ibp:
    #         l_x.repeat(1, other_lenth).reshape(-1),
    #         u_x.repeat(1, other_lenth).reshape(-1),
    #         l_y.reshape(-1).repeat(self_lenth),
    #         u_y.reshape(-1).repeat(self_lenth),
    #         prod1 = l_x * l_y
    #         prod2 = l_x * u_y
    #         prod3 = u_x * l_y
    #         prod4 = u_x * u_y
    ...
    #
    #         zeros = torch.zeros(l_x.shape).to(self.device)
    #
    #         return zeros, zeros, l, zeros, zeros, u
    #
    #     eps=
    #     W=np.eye(d_model)
    #     # Diagonal matrix to be optimized
    #     d_var = cvx.Variable(d_model, nonneg=True)
    #     # PSD constraint
    #     W_param = cvx.Parameter((d_model, d_model))
    #     cons_PSD = cvx.diag(d_var) + W_param >> 0
    #     # Objective function
    #     eps2_param = cvx.Parameter(d_model, nonneg=True)
    #     obj = cvx.Minimize(eps2_param @ d_var)
    #     # Optimization problem
    #     prob = cvx.Problem(obj, [cons_PSD])
    #     prob.is_dpp()
    #     for bnd in [0, 1]:
    #         # Instantiate W
    #         W_param.value = W if bnd == 0 else -W
    #         # Iterate over positions
    #         for pos in range(self_lenth):
    #             # Instantiate objective function weights
    #             eps2_param.value = eps[pos] ** 2
    #
    #             # Solve problem and save solution
    #             prob.solve()
    #             # print(obj.value)
    #             gapMean[pos, pos, bnd] = obj.value / 3
    #             D[pos, pos, bnd] = d_var.value
    #             # Linear term coefficients
    #             b[pos, pos, bnd] = -D[pos, pos, bnd] * m[pos]
    #             # Constant term
    #             c[pos, pos, bnd] = -(b[pos, pos, bnd] @ m[pos] + obj.value) / 2
    ...
    With a relaxation on b[k][j], so that b[k][j] \in [l[k][j], r[k][j]]:
        ...
        Most coarse/loose first:
            ...
            D^l[i][j][m] = -d^u[i][j]
    """
    def dot_product(self, other, debug=False, verbose=False, lower=True, upper=True,bi=False):
        if self.dim_in == 1:
